...
def index(request: Request, state: str = Form(...), categories: str = Form(...)):
    response = requests.get(f'https://api-recomendaciones.onrender.com/?state={state}&categories={categories}')
    data = response.json()
...
def sentimiento_index(request: Request, state: str = Form(...), categories: str = Form(...)):
    response = requests.get(f'https://api-recomendaciones.onrender.com/sentimiento_cercano?state={state}&categories={categories}')
    data = response.json()
